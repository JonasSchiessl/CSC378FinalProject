...
@export var enemy_type: String = "tower_destroyer_rat"
@export var currency_reward: int = 20
...
func _physics_process(delta: float) -> void:
	...
	# If the target tower has been destroyed, find another target
	if target in get_tree().get_nodes_in_group("towers"):
...
func perform_attack(direction: Vector2) -> void:
	print("TOWER RAT IS TYING TO ATTACK!") 
